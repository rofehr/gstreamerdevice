...
public:
   ...
   */
    ...
    {
/*      
      g_printerr("push_to_buffer() Begin \n");
...
/*      
	...
	memset(map.data, Data[i], 1);
        gst_buffer_unmap(buffer, &map);
      
      
        //g_printerr("push_to_buffer() '%d: pushing buffer for pointer %p, %p' \n",i, map.data, buffer);
	gst_app_src_push_buffer(GST_APP_SRC(data.src), buffer);
      }
...
*/      
	...
	return Length;
      ...
      {
	//g_printerr("ilive_stream_count %d \n",ilive_stream_count);
	 if(!live_stream_is_runnig)
	 {
            //g_printerr("StartReplay Begin \n");
